...
# ── Load MD data ───────────────────────────────────────────────────────────────
md_data = np.loadtxt("mu_rho_M_co2.txt")
mu_real, rho_SI, M_SI = md_data.T
...
axes[0].scatter(mu_real, rho_SI, s=20, color="black", zorder=5, label="Data")
axes[0].plot(mu_grid/kcal_to_j, rho_fwd, color="tab:blue", label="forward")
axes[0].plot(mu_from_rho/kcal_to_j, rho_back, color="tab:orange",
             linestyle="--", label="rho → mu → rho")
axes[0].set_xlabel("mu (kcal/mol)")
axes[0].set_ylabel("rho (1/m³)")
axes[0].set_title("rho(mu) — forward + inverse consistency")
axes[0].legend()

# Right: M forward only
axes[1].scatter(mu_real, M_SI, s=20, color="black", zorder=5, label="Data")
axes[1].plot(mu_grid/kcal_to_j, M_fwd, color="tab:green", label="forward")
axes[1].set_xlabel("mu (kcal/mol)")
