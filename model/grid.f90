program grid_model
    ...
    real(8), allocatable :: mu_vs_M(:)
    real(8), allocatable :: M_vs_mu(:)

    real(8), parameter :: kcal_to_j = 4184.0d0

    ! Load coefficients
    call load_coeffs("../data/T84/mu_vs_rho_fit.txt", mu_vs_rho, deg1)
    call load_coeffs("../data/T84/rho_vs_mu_fit.txt", rho_vs_mu, deg2)
    call load_coeffs("../data/T84/mu_vs_M_fit.txt", mu_vs_M, deg3)
    ...
    ! Parameters
    time_step = 1e-12 ! s (initial timestep)
    Na = 6.022e23 ! mol-1
    ...
    left_mu = -3.5d0 * kcal_to_j
    right_mu = -1d0 * kcal_to_j
    mu_mode = 2 ! Pick the initial chemical potential profile (1=linear, 2=left, 3=right)

    ! For sanity check, generate the curves that have been imported from MD
    call generate_tables(left_mu, right_mu, 10, rho_vs_mu, deg2, &
                         M_vs_mu, deg4, mu_vs_rho, deg1, mu_vs_M, deg3)
